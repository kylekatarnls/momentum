                    ...
                    momentum.getItem('animals', id, function (dog) {
                        ...
                        momentum.listenCollection('animals', function () {
                            ...
                            var offItem = dog.onUpdate(function (event) {
                                calls.push('item');
                                expect(event.update.$set.color).toBe(color);
                                offItem();
                                setTimeout(function () {
                                    dog.stop();
                                    end();
                            ...
                            var offMomentum = momentum.on(function (events) {
                                calls.push('momentum');
                                expect(events[0].args[3].update.$set.color).toBe(color);
                                offMomentum();
                                setTimeout(end, 100);
                            });
                            ...
                            var quit = function () {
                                ...
                                expect(calls.join(',')).toBe('item,collection,momentum', 'item callback should be called first, then collection callback, then momentum callback');
                                momentum.quit(done);
                            };
