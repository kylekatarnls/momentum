(function (root, factory) {
    ...
    Momentum.prototype.getCollection = function (args, getConfigIdentity, callback) {
        this.listenCollection(args[0]);

        return this.find(args, function (data) {
            callback(new Collection(this, args, getConfigIdentity, data.result));
        }.bind(this));
    };

    var arrayPrototype = Array.prototype;

    function Collection(momentum, args, getConfigIdentity, data) {
        this.momentum = momentum;
        this.args = args;
        var changeEvents = [];
        this.changeEvents = changeEvents;
        arrayPrototype.push.apply(this, data.map(function (properties) {
            return new Item(this, getConfigIdentity(properties), properties);
        }.bind(this)));
        this._stop = momentum.on(function (events) {
            events.forEach(function (event) {
                if (event.listen.collection === args[0] && !event.listen.filter && !event.listen.id) {
                    changeEvents.forEach(function (callback) {
                        var params = event.args.slice();
                        params.unshift(callback);
                        execute.apply(this, params);
                    });
                }
            });
        });
    ...
    Collection.prototype.onChange = function (callback) {
        ...
        return function () {
            this.changeEvents = this.changeEvents.filter(function (c) {
                return c !== callback;
            })
        }.bind(this);
    };
    ...
    Collection.prototype.stop = function () {
        this._stop();
        this.momentum.stopListenCollection(this.args[0]);
    };

    Collection.prototype.emit = function (method, args, callback) {
        var params = [].slice.call(args);
        params.unshift(this.args[0]);
        this.momentum[method](params, callback);
    };
     ...
     * @returns this
     */
    Collection.prototype.insertMany = function (args, callback) {
        return this.emit('insertMany', args, callback);
    };
     ...
     * @returns this
     */
    Collection.prototype.updateOne = function (item, update, callback) {
        return this.emit('updateOne', [item, update], callback);
    };
     ...
     * @returns this
     */
    Collection.prototype.updateMany = function (args, callback) {
        return this.emit('updateMany', args, callback);
    };
    ...
    Item.prototype.onChange = function (callback) {
        this.collection.onChange(function (events) {
            callback(events);
        });
    };

    Item.prototype.update = function (properties, callback) {
        this.collection.updateOne(this.identity, properties, callback)
    };

    Item.prototype.remove = function (callback) {
        this.collection.remove(this.identity, callback);
    };
