(function (root, factory) {
    ...
    /**
     * Copy object properties in another object.
     * @param {Object} source
    ...
    /**
     * Abort connection to momentum server after this delay.
     * @type {number}
     */
    Momentum.prototype.connectTimeout = 10000;
    ...
    Momentum.prototype.onReady = function (callback) {
        if (this.isReady) {
            ...
            var retry = function () {
                ...
                var delay = this.connectTimeout;
                var timeout = setTimeout(function () {
                    retry = function () {};
                    xhr.abort();
                    this.markAsReady(new Error('Connection timeout (' + delay + 'ms) expired'));
                }.bind(this), delay);
            }.bind(this);
            retry();
        }
    ...
    Momentum.prototype.quit = function (callback) {
        return this.onReady(function () {
            ajax.get(this.url + this.getUrlPrefix() + 'quit', {
                token: this.token
            }, function (data) {
    ...
    var itemList = function (items) {
        return (items || []).filter(function (properties) {
            return properties;
        });
    };

    var getRegistryForIds = function (event) {
        var registry = {};
        var data = event.args[1];
        itemList(data.ids || [data.id]).forEach(function (id) {
            registry[id] = true;
        });
            ...
            if (event.listen.collection === name && !event.listen.filter) {
                switch (event.args[0]) {
                    ...
                    case 'update-collection':
                        var update = event.args[1].update;
                        var updateRegistry = getRegistryForIds(event, update);
                        this.forEach(function (item) {
                            if (updateRegistry[item._id]) {
                                extend(item, update.$set);
                            }
                        });
                        break;
                    case 'remove-item':
                    case 'remove-collection':
                        var removeRegistry = getRegistryForIds(event, true);
                        for (var i = 0; i < this.length; i++) {
                            var item = this[i];
                            if (removeRegistry[item._id]) {
                                this.splice(i, 1);
                                i--;
                                forEach(item, function (key) {
                                    if (key !== '_id') {
                                        delete item[key];
                                    }
                                });
                ...
                this._changeEvents.forEach(function (callback) {
                    execute.apply(this, [callback, event.args[1]]);
                });
            }
        }.bind(this));
    }
    ...
    Collection.prototype.append = function (data) {
        itemList(data).forEach(function (properties) {
            this.add(new Item(this, {_id: properties._id}, properties));
        }.bind(this));
     ...
     * to remote it.
     *
     * @param {Function} callback action triggered on change
     * @param {String}   name     optional event name filter
     *
     * @returns {Function}
     */
    Collection.prototype.onChange = function (callback, name) {
        ...
        this._changeEvents.push(wrapper);
    ...
    /**
     * Add a listener on insert event and return a callback
     * to remote it.
     *
     * @param {Function} callback
     *
     * @returns {Function}
     */
    Collection.prototype.onInsert = function (callback) {
        return this.onChange(callback, 'insert');
    };
    ...
    Collection.prototype.onUpdate = function (callback) {
        return this.onChange(callback, 'update');
    };

    /**
     * Add a listener on remove event and return a callback
     * to remote it.
     *
     * @param {Function} callback
     *
     * @returns {Function}
     */
    Collection.prototype.onRemove = function (callback) {
        return this.onChange(callback, 'remove');
    };
     ...
     * @param {Function} callback
     */
    Collection.prototype.stop = function (callback) {
        this._stop();
        this.getMomentum().stopListenCollection(this.getName(), null, callback);
    };
     ...
     * @returns {Function}
     */
    Item.prototype.onRemove = function (callback) {
        return this.onChange(callback, 'onRemove');
    };
    ...
    /**
     * Return true if an item is removed from its collection.
     *
    ...
    /**
     * @alias Item.prototype.isRemoved
     */
    Item.prototype.isDeleted = Item.prototype.isRemoved;
