(function (root, factory) {
    ...
    var getRegistryForIds = function (event, value) {
        var registry = {};
        itemList(event.args[1].ids).forEach(function (id) {
            registry[id] = value;
        });
            ...
            if (event.listen.collection === name && !event.listen.filter) {
                switch (event.args[0]) {
                    case 'insert':
                        ...
                        break;
                    case 'update-item':
                        var _item = event.args[3].item;
                        this.forEach(function (item) {
                            if (item._id === _item._id) {
                                extend(item, event.args[3].update.$set);
                            }
                        });
                    case 'update-collection':
                        var registry = getRegistryForIds(event, event.args[1].update);
                        this.forEach(function (item) {
                            if (registry[item._id]) {
                                extend(item, registry[item._id].$set);
                            }
                        });
                        break;
                    case 'remove-item':
                    ...
                    case 'remove-collection':
                        var registry = getRegistryForIds(event, true);
                        this.forEach(function (item) {
                            if (registry[item._id]) {
                                forEach(item, function (key) {
                                    if (key !== '_id') {
    ...
    Collection.prototype.append = function (data) {
        arrayPrototype.push.apply(this, itemList(data).map(function (properties) {
